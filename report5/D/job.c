		...
		while(cur_job != NULL) {
			...
			if ((pid = fork()) == 0) {
				...
				if (pr->output_redirection != NULL) {
					if (pr->output_option == APPEND) {
						out_fd = open(pr->output_redirection, O_CREAT|O_RDWR|O_APPEND, 0666);
					} else {
						out_fd = open(pr->output_redirection, O_CREAT|O_RDWR|O_TRUNC, 0666);
					}
					if (out_fd < 0) {
						perror("open() failed\n");
						exit(EXIT_FAILURE);
